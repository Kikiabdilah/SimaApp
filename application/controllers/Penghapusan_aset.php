...
class Penghapusan_aset extends CI_Controller
{
    ...
    public function penghapusan_aset()
    {
        ...
        // var_dump($data['matriks_nilai_while']);
        // exit;
    ...
    public function get_data_m_kd()
    {
        ...
        $data2 = $this->Pengadaan_aset_model->get_pengadaan();
        foreach ($data as $val) {
            foreach ($data2 as $val1) {
                if ($val['kd_brg'] == $val1['kd_brg']) {
                    $aa[] = [
                        'id_rengking' => $val['id_rengking'],
                        'kd_rengking' => $val['kd_brg'],
                        'rangking' => $val['rangking'],
                        'id_brg' => $val['id_brg'],
                        'kd_brg' => $val['kd_brg'],
                        'nm_brg' => $val['nm_brg'],
                        'thn_pengadaan' => $val1['thn_pengadaan']
                    ];
                }
            }
        }
        array_multisort(array_column($aa, 'thn_pengadaan'), SORT_ASC, $aa);
        echo json_encode($aa);
    }
    ...
    {
        $data['rengking']      = $this->Penghapusan_aset_model->get_rengking();
        $this->load->view('Manager/penghapusan_aset', $data);
    }

    public function penghapusan_aset_kep()
    {
        $data['rengking']      = $this->Penghapusan_aset_model->get_rengking();
        $this->load->view('Dirut/penghapusan_aset', $data);
    }
