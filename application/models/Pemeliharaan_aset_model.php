...
class Pemeliharaan_aset_model extends CI_Model
{
    function get_data_aset()
    {
        return $this->db->query(
            "SELECT
                a.id_brg, a.kd_brg, a.nm_brg, a.harga, a.kondisi, a.umr_ekonomis, a.nli_sisa, a.stts_pemeliharaan, a.thn_beli,
    ...
    function get_data_pemeliharaan()
    {
        return $this->db->query(
            "SELECT
                a.id_pemeliharaan, a.id_brg, a.kd_brg, a.nm_brg, a.kondisi_brg,
                a.harga, a.umr_ekonomis, a.nli_sisa, a.stts_approval, a.stts_approval_kep, b.thn_beli, a.ket,
    ...
    function aksi_pemeliharaan_wk($id_pemeliharaan, $id_brg, $kd_matriks, $stts_approval, $nilai_buku_bln, $sisa_umr_ekonomis, $thn_beli)
    {
        $f = date('Y') - $thn_beli;
        // echo $nilai_buku_bln;
        // exit;
        if ($f  <= 1) {
            $akumulasi_penyusutan = 1;
        } elseif ($f  <= 2) {
            $akumulasi_penyusutan = 2;
        } elseif ($f  <= 3) {
            $akumulasi_penyusutan = 3;
        } elseif ($f  <= 4) {
            $akumulasi_penyusutan = 4;
        } elseif ($f  >= 5) {
            $akumulasi_penyusutan = 5;
        } else {
        ...
        if ($stts_approval == 2) {
            ...
            if ($sisa_umr_ekonomis <= 0) {
                ...
                $sisa_umr_eko = 4;
            } elseif ($sisa_umr_ekonomis <= 5) {
                $sisa_umr_eko = 3;
            } elseif ($sisa_umr_ekonomis <= 10) {
                $sisa_umr_eko = 2;
            } elseif ($sisa_umr_ekonomis <= 20) {
                $sisa_umr_eko = 1;
            } else {
                $sisa_umr_eko = 0;
            }
            ...
            if ($nilai_buku_bln >= 0 and $nilai_buku_bln <= 500000) {
                $nilai_buku = 10;
            } elseif ($nilai_buku_bln >= 500001 and $nilai_buku_bln <= 1000000) {
                $nilai_buku = 9;
            } elseif ($nilai_buku_bln >= 1000001 and $nilai_buku_bln <= 1500000) {
                $nilai_buku = 8;
            } elseif ($nilai_buku_bln >= 1500001 and $nilai_buku_bln <= 2000000) {
                $nilai_buku = 7;
            } elseif ($nilai_buku_bln >= 2000001 and $nilai_buku_bln <= 2500000) {
                $nilai_buku = 6;
            } elseif ($nilai_buku_bln >= 2500001 and $nilai_buku_bln <= 3000000) {
                $nilai_buku = 5;
            } elseif ($nilai_buku_bln >= 3000001 and $nilai_buku_bln <= 3500000) {
                $nilai_buku = 4;
            } elseif ($nilai_buku_bln >= 3500000 and $nilai_buku_bln <= 4000000) {
                $nilai_buku = 3;
            } elseif ($nilai_buku_bln >= 4000001 and $nilai_buku_bln <= 4500000) {
                $nilai_buku = 2;
            } elseif ($nilai_buku_bln >= 4500001) {
                $nilai_buku = 1;
            } else {
                $nilai_buku = 0;
            }
    ...
    function print_pemeliharaan_aset_ex($tgl_awal, $tgl_akhir, $kategori)
    {
        if ($tgl_awal == "" and $tgl_akhir == "") {
            ...
            $data = $this->db->query(
                "SELECT
                    a.kd_brg, a.nm_brg, a.kondisi_brg, a.harga, a.umr_ekonomis, a.nli_sisa, a.ket, b.perolehan, b.thn_beli, b.satuan_brg,
                    (
